...
def get_local_expl_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser('Explain a prediction of a ProtoRS model')
    parser.add_argument('--model',
                        type=str,
                        help='Directory to trained ProtoRS model')
    parser.add_argument('--log_dir',
                        type=str,
                        default='./runs/run_protors',
                        help='The directory in which results should be logged. Should be same log_dir as loaded ProtoRS')
    parser.add_argument('--dataset',
                        type=str,
                        default='CUB-200-2011',
                        help='Data set on which the ProtoRS model was trained')
...
def explain_local(args):
    ...
    # Load trained ProtoRS
    model = ProtoRS.load(args.model).to(device=device)
