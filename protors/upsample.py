...
# slightly changed from ProtoTree
def upsample(model: ProtoRS, project_info: dict, project_loader: DataLoader, folder_name: str, args: argparse.Namespace, log: Log):
    dir = os.path.join(os.path.join(args.log_dir, args.dir_for_saving_images), folder_name)
    if not os.path.exists(dir):
        os.makedirs(dir)
    with torch.no_grad():
        sim_maps, project_info = get_similarity_maps(model, project_info, log)
        ...
        for j in range(model.num_prototypes):
            ...
            if len(torch.unique(model.prototype_layer.prototype_vectors[0:j+1], dim=0)) > len(torch.unique(model.prototype_layer.prototype_vectors[0:j], dim=0)):
                ...
                x.save(os.path.join(dir,'%s_original_image.png'%str(prototype_idx)))
...
# mostly copied from ProtoTree
def get_similarity_maps(model: ProtoRS, project_info: dict, log: Log = None):
    log.log_message("\nCalculating similarity maps (after projection)...")
    
    sim_maps = dict()
    for j in project_info.keys():
        nearest_x = project_info[j]['most_similar_input']
        with torch.no_grad():
            _, similarities_batch = model.forward_partial(nearest_x)
            sim_maps[j] = similarities_batch[0,j,:,:].cpu().numpy() # the only change compared to ProtoTree

        del nearest_x
        del project_info[j]['most_similar_input']
