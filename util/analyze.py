...
def analyze_output_shape(tree: ProtoRS, trainloader: DataLoader, log: Log, device):
    with torch.no_grad():
        # Get a batch of training data
        xs, ys = next(iter(trainloader))
        xs, ys = xs.to(device), ys.to(device)
        log.log_message("Image input shape: "+str(xs[0,:,:,:].shape))
        log.log_message("Features output shape (without 1x1 conv layer): "+str(tree.net(xs).shape))
        log.log_message("Convolutional output shape (with 1x1 conv layer): "+str(tree.add_on(tree.net(xs)).shape))
